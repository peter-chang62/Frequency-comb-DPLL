...
class ConfigRPSettingsUI(Qt.QWidget):
    ...
    def loadParameters(self):
        # print("ConfigurationRPSettingsUI::loadParameters()") Load the default parameters from the selected xml file
        # (select by the devices_data dictionnary in the controller class)
    ...
    def setClkSelect(self, checked=False):
        if self.qradio_external_clk.isChecked():
            ...
            self.sl.setADCclockPLL(f_int, self.qradio_external_clk.isChecked(), CLKFBOUT_MULT,
                                   CLKOUT0_DIVIDE)  # calling this crashes the thing...
