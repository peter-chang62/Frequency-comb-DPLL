...
class controller(object):
    ...
    def connectionGUI(self):
        ...
        self.initial_config = initialConfiguration(self.sl.dev, self, self.devices_data, strBroadcastAddress,
                                                   strFPGAFirmware, strCPUFirmware)
    ...
    def initUI(self):
        ...
        try:
            ...
            custom_style_sheet = (
                        '#MainWindow {color: white; background-color: %s;}' % self.devices_data[self.strSelectedSerial][
                    'color'])
        except KeyError:
            custom_style_sheet = ''
        ...
        self.divider_settings_window = DisplayDividerAndResidualsStreamingSettingsWindow(self.sl, self.sp,
                                                                                         clk_divider_modulus=67e3,
                                                                                         bDividerOn=0, bPulses=0,
                                                                                         custom_style_sheet=custom_style_sheet,
                                                                                         custom_shorthand=custom_shorthand)

        # Optical lock window
        self.xem_gui_mainwindow2 = XEM_GUI_MainWindow(self.sl, custom_shorthand + ': Optical lock', 1,
                                                      (False, True, True), self.sp, custom_style_sheet,
                                                      self.strSelectedSerial)

        # CEO Lock window
        self.xem_gui_mainwindow = XEM_GUI_MainWindow(self.sl, custom_shorthand + ': CEO lock', 0, (True, False, False),
        ...
        self.freq_error_window1 = FreqErrorWindowWithTempControlV2(self.sl, 'CEO beat in-loop counter', self.sp, 0,
                                                                   strNameTemplate, custom_style_sheet, 0,
                                                                   self.xem_gui_mainwindow)
        self.freq_error_window2 = FreqErrorWindowWithTempControlV2(self.sl, 'Optical beat in-loop counter', self.sp, 1,
                                                                   strNameTemplate, custom_style_sheet,
                                                                   temp_control_port, self.xem_gui_mainwindow2)
        ...
        self.dither_widget0 = DisplayDitherSettingsWindow(self.sl, self.sp, 0, modulation_frequency_in_hz='1e3',
                                                          output_amplitude='1e-3', integration_time_in_seconds='0.1',
                                                          bEnableDither=True, custom_style_sheet=custom_style_sheet)
        self.dither_widget1 = DisplayDitherSettingsWindow(self.sl, self.sp, 1, modulation_frequency_in_hz='5.1e3',
                                                          output_amplitude='1e-3', integration_time_in_seconds='0.1',
                                                          bEnableDither=True, custom_style_sheet=custom_style_sheet)
        ...
        self.RP_Settings = ConfigRPSettingsUI(self.sl, self.sp, self, custom_style_sheet=custom_style_sheet,
                                              custom_shorthand=custom_shorthand)
    ...
    def loadDefaultValueFromConfigFile(self, strSelectedSerial, bSendToFPGA=True):
        try:
            ...
            # print('Warning: Could not parse config file for FPGA serial: %s, loading values from file %s' % (self.initial_config.strSelectedSerial, strFileDefaultConfig))
            print('Warning: Could not parse config file for FPGA serial: %s, loading values from file %s' % (
    ...
    def setCustomStyleSheet(self, strSelectedSerial):
        ...
        try:
            # custom_style_sheet = ('#MainWindow {color: white; background-color: %s;}' % self.devices_data[self.initial_config.strSelectedSerial]['color'])
            custom_style_sheet = (
                        '#MainWindow {color: white; background-color: %s;}' % self.devices_data[strSelectedSerial][
                    'color'])
        except KeyError:
            custom_style_sheet = ''
    ...
    def getActualValues(self, strSelectedSerial, ip_addr="192.168.0.150", port=5000):
        ...
        self.loadDefaultValueFromConfigFile(strSelectedSerial,
                                            False)  # read xml file to update some values. False means not updating the FPGA
    ...
    def reconnectionAttempt(self):
        ...
        if self.reconnection_attempts == 10:
            ...
            print(
                "%d failed reconnection attempts. Switching to 10 secs between attempts." % self.reconnection_attempts)
            self.timerReconnect.stop()
