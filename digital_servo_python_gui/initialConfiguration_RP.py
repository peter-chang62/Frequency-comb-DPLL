...
class initialConfiguration(QtWidgets.QDialog):
    ...
    def __init__(self, dev, controller, devices_data={}, strBroadcastAddress="192.168.2.255", strFPGAFirmware='',
                 strCPUFirmware=''):
    ...
    def initUI(self):
        ...
        self.qcombo_serial.setMinimumContentsLength(
            100)  # I can't figure out how to make it scale correctly with content so we'll make it big enough...
    ...
    def readSelectedFPGA(self):
        ...
        if self.qradio_usefromlist.isChecked():
            ...
            try:
                ...
                self.strSelectedSerial = strMAC.replace(':',
                                                        '')  # this is just for legacy compatibility, when we had actual serial numbers
    ...
    def okClicked(self):
        ...
        self.strFPGAFirmware = (
            self.qedit_firmware.text())  # the str() is to convert the QString to a normal Python string object
        self.strCPUFirmware = (
            self.qedit_software.text())  # the str() is to convert the QString to a normal Python string object
        ...
        self.killTimer(
            self.timerID)  # is it guaranteed that no timerEvent will be called after this line? because we delete our reference to udp_discovery, which is used in the timer event
    ...
    def programFPGAClicked(self):
        ...
        self.logger.info(
            'Red_Pitaya_GUI{}: Programming FPGA ({}) with new bitfile'.format(self.logger_name, self.strSelectedPort))
        ...
        self.dev.write_file_on_remote(strFilenameLocal=str(self.qedit_firmware.text()),
                                      strFilenameRemote='/opt/red_pitaya_top.bit')
    ...
    def programCPUClicked(self):
        ...
        self.logger.info(
            'Red_Pitaya_GUI{}: Programming CPU ({}) with new file'.format(self.logger_name, self.strSelectedPort))

        # connect to the selected RedPitaya
        self.dev.OpenTCPConnection(self.strSelectedIP, self.strSelectedPort)
        # send new monitor-tcp version
        self.dev.write_file_on_remote(strFilenameLocal=self.qedit_software.text(),
                                      strFilenameRemote='/opt/monitor-tcp-new')
...
def main():
    ...
    dev = RP_PLL_device()
    initial_config = initialConfiguration(dev, None, devices_data=devices_data, strFPGAFirmware=strFPGAFirmware,
                                          strCPUFirmware=strCPUFirmware)
